...
public:
    ...
    /**
     * Copy constructor for a quadtree since
     * quadtree allocate dynamic memory
     *
     * @see quadtree.cpp
...
private:
    ...
    void clearQuadTree(Node *croot);

    Node *copyQuadTree(Node *croot);

    void pruneQuadTree(Node *croot, const int tol);

    int pruneSizeQuadTree(Node *croot, const int tol) const;
};
